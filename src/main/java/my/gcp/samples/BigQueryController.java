...
public class BigQueryController {
     ...
     * @throws IOException if the file is unable to be loaded.
     */
    @GetMapping("/getTableContent")
    @ResponseBody
    public TableResult getTableContent(@RequestParam("tableName") String tableName)
            throws IOException, InterruptedException {

        String query = String.format("SELECT * FROM my_dataset.%s;", tableName);
        QueryJobConfiguration queryConfig =
                QueryJobConfiguration.newBuilder(query).build();

        // Run the query using the BigQuery object
        final TableResult result = bigQuery.query(queryConfig);
...
//            }
    ...
    @GetMapping("/update")
    @ResponseBody
    public TableResult update() throws InterruptedException {
        String query = String.format("update * FROM my_dataset.%s;", "new_table");
        QueryJobConfiguration queryConfig =
                QueryJobConfiguration.newBuilder(query).build();
